...
var nowPlayingLabel = document.createElement("span");
...
var notes = [];       // array of {el, x, y, speed} el for elements
var notesTimer = null;
function spawnNote() {
  var symbols = ["♪", "♫", "♩", "♬"];
  var el = document.createElement("div");
  el.className = "note colorcycle";  //  will use CSS animation to cycle
...
var currentAudioSrc = "";
// Function to play audio in different pages with fading when music change or goes back to loop
...
// Fade out the music and stop the music
function stopMusic() {
  fadeVolume(0, 150);
  setTimeout(function () {
    bgm.pause();
...
function showHome() {
  ...
  openId = null;
  clearActiveButton(); // clears highlight
  stopMusic();
}
...
function showSection(id) {
  ...
  // ensure minigame is ready with a fresh round when opened
  if (id === 'minigame' && typeof giEnsureFreshRound === 'function') giEnsureFreshRound();
}

// ---- Avoid function definitions inside the loop ----
...
feedbackBtn.addEventListener("click", function (e) {
  ...
  if (!status) {
    ...
    // removed ARIA role to satisfy "no aria"
  ...
  lastFeedbackTime = setInterval(function () {
    ...
    if (remainingTime > 0) {
      ...
      // Re-open controls and focus for convenience
      textarea.classList.add("textarea--active");
      feedbackBtn.classList.add("feedback-btn--active");
  ...
  // Keep a clean slate for the next message
  var selected = emoji.querySelector("[data-selected]");
  if (selected) selected.removeAttribute("data-selected");
  textarea.value = "";
});
...
// - Event delegation, class toggles, CSS var updates
// =========================================================

// Grab DOM references (GetElementById / QuerySelector)
...
// Answer key: radio group name -> { answer, explain }
var ANSWER_KEY = {
...
// Guard against missing elements if quiz not on page
if (quizEl && btnSubmit && btnRetry && scorebox) {
  ...
  btnSubmit.addEventListener('click', function () {
    ...
    // Update CSS properties / classes based on score (visual feedback)
  ...
  btnRetry.addEventListener('click', function () {
    ...
    // Reset CSS accent back to default (JS -> CSS property)
...
// ---- helper to avoid callbacks created inside loops ----
...
function gradeFromFormData(fd, keyMap) {
  ...
  // Use FormData.forEach (loop + conditions)
...
// Mark a fieldset as pending (visual prompt)
...
// Clear all per-question markings
...
// Compute score visuals: set CSS variable + toggle container classes
function applyScoreStyling(correct, total) {
  var ratio = total ? (correct / total) : 0;
  // Update an accent CSS var based on performance (JS -> CSS)
...
// Utility: get fieldset from any descendant
...
// Utility: get radio group name from a fieldset
function getGroupName(fs) {
  var input = fs.querySelector('input[type="radio"]');
  return input ? input.name : '';
}

// Utility: find the fieldset by the radio group name
...
// Guard (allow page to omit this section without errors)
...
function startRound() {
  ...
  // Show the correct sprite frame via CLASS (no inline background changes)
  setSpriteByClass(pick.className);

  // Build randomized choices (1 correct + 3 distractors)
  buildChoices(answerId);

  // Set audio to the instrument file; safe-guard if missing
  ...
  // Start countdown + note animation (timed events & JS animation using CSS positions)
...
function setSpriteByClass(instrClass) {
  // Remove all instrument classes, then add the one we want
  giSprite.classList.remove('guitar', 'timpani', 'trumpet', 'harpsichord', 'oboe', 'violin');
  giSprite.classList.add(instrClass);
}

// Simple "bop" using left/top (position-based JS animation)
...
// Floating notes using left/top via rAF
...
// We DO NOT auto-start a new round here (so reset to Home stays quiet).
...
(function () {
  ...
  img.id = "qrImg";
  img.src = "images/qr-code.png"; // QR image here
